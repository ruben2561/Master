    ...
    try:
        with open("sampleData/solar_data_year_processed.csv", "r") as file:
            ...
            for point in values_list:
                period_end = datetime.strptime(point["time_value"], "%Y-%m-%dT%H:%M:%S")  # Convert string to datetime
                pv_list.append({"pv_power_value": float(point["pv_power_value"]), "time_value": period_end, })
            return pv_list
...
def calculate_pv_power(forecast_list, latitude, longitude, number_of_panels, area, azimuth, tilt, efficiency_panels,
    ...
    poa_globals = \
    pvlib.irradiance.get_total_irradiance(surface_tilt=tilt, surface_azimuth=azimuth, dni=dnies, ghi=ghies, dhi=dhies,
    ...
    for i, data_point in enumerate(forecast_list):
        pv_list.append({"pv_power_value": pv_estimates[i], "temperature_out": float(data_point["temp"]),
...
def process_solar_data(latitude, longitude, start_date, number_of_panels, area, azimuth, tilt, efficiency_panels,
    ...
    for data_point in pv_list:
        ...
        data_points.append(
            {"pv_power_value": data_point['pv_power_value'], "temperature_out": data_point["temperature_out"],
                "time_value": data_point['time_value'], "soc_value": 0, "charge_value": 0, "discharge_value": 0,
                "price_battery_use": 0, "grid_injection": 0, "grid_offtake": 0, "power_usage_value": 0,
                "price_injection": 0, "price_offtake": 0, "ev_charge_value": 0, "heat_pump_value": 0, })
